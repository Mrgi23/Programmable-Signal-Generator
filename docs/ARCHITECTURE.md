# Programmable Signal Generator - System Architecture
...
A key challenge in digital signal processing is compensating for artifacts introduced by the digital-to-analog conversion process. The **$sinc(x)$ compensation filter** corrects for signal distortions caused by the inherent frequency response of the DAC, ensuring that the output signal maintains its intended shape and spectral properties.

## **Digital-to-Analog Converter**
Since this is a fully software-based system, the **DAC** simulation emulates the behavior of a real-world **digital-to-analog converter**. Two output modes are available. Each mode influences how the signal is represented in its final form:
