...
def test_complex_mixer_valid_output(complex_mixer):
    ...
    # Test the result.
    spec = [int(f - f_shift), int(f + f_shift), int(fs - f - f_shift), int(fs - f + f_shift)]
    assert(I_out.size == I.size), "Invalid size of the shifted signal."
    assert(np.allclose(I_out[spec], fs // 4, atol=5e-2)), "Spectral components must be at f - f_shift and f + fshift."
    assert(np.allclose(np.delete(I_out, spec), 0, atol=5e-2)), "Noise must be close to 0."
